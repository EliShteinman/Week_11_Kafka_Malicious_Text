...
class SentimentAnalyzer:
    def __init__(self, path_download: str = None):
        nltk_dir = path_download if path_download else os.path.join(os.getcwd(), "nltk_data")
        try:
    ...
    def get_sentiment_label(self, text: str, positive_threshold: float = 0.5, negative_threshold: float = -0.5) -> str:
        if not text or not isinstance(text, str):
            logger.debug("Empty or invalid text for sentiment analysis")
            return "neutral"
        score = self.get_sentiment_score(text)
        return self.convert_to_sentiment_label(score, positive_threshold, negative_threshold)

    def convert_to_sentiment_label(self, score: float, positive_threshold: float, negative_threshold: float) -> str:
